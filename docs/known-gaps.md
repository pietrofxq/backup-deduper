...
| # | location | issue |
|---|----------|-------|
| ~~SG-1~~ | ~~[src/orchestrator/sanityGuard.ts](../src/orchestrator/sanityGuard.ts)~~ | ~~`checkSanityGuard` returns `passed: true` when no primary collection is set.~~ ✅ closed by M15: `checkSanityGuard` now returns `passed: false` with `code: 'no_primary_set'` when there's no primary AND at least one action would fire; vacuous (zero-action) runs still pass. See the `if (!primary)` branch (~lines 56–86 at time of writing). |
